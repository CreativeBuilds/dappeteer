...
export type Dappeteer = {
  ...
  addNetwork: (url: string, name: string, chainId: number, ticker: string) => Promise<void>
...
export async function getMetamask(
  ...
  return {
    ...
    addNetwork: async (url: string, name: string, chainId: number, ticker: string) => {
      await metamaskPage.bringToFront()
      ...
      /* networkIndex is the index of Custom RPC button on the list of network options */
      ...
      /* Broke here */ // was because of wrong id tag
      const newRPCInput = await metamaskPage.waitForSelector('input#rpc-url')
      await newRPCInput.type(url)

      const newNameInput = await metamaskPage.waitForSelector('input#network-name')
      await newNameInput.type(name)
      
      const newIdInput = await metamaskPage.waitForSelector('input#chainId')
      await newIdInput.type(String(chainId))

      const networkTicker = await metamaskPage.waitForSelector('input#network-ticker')
      await networkTicker.type(ticker)

      /* Before saving add network ChainID and Name */
      const saveButton = await metamaskPage.$('button.btn-secondary:nth-child(2)')
